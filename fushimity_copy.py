...
class Explosion:
    def __init__(self,bomb):
        self.center=None
        self._life=50
        expgif=pg.image.load("ex03/fig/explosion.gif")
        self._imgs=[
            expgif,
            pg.transform.flip(expgif,False,True),
            pg.transform.flip(expgif,True,True),
            pg.transform.flip(expgif,True,False)
        ]
        self._rect=bomb.center
    def update(self,screen):
        self._life-=1
        screen.blit(self._imgs[self._life%4],self.center)
...
def main():
    ...
    exps=[Explosion(bombs[i]._rct) for i in range(NUM_OF_BOMBS)]
    beam = None
